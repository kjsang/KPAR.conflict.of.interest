...
# 1. 데이터 불러오기 --------------------------------------------
read_csv("1.data//1.1.news.csv") -> data # 데이터 불러오기: 전체 언론기사 중 지면기사 수집
data # 데이터 확인
...
data_prep_ver2 %>% write_excel_csv("4.output//data_prep_ver2.csv")
...
data_prep_ver3 %>% write_excel_csv("4.output//data_prep_ver3.csv") # 중간 저장
...
data_prep_ver4 %>% write_excel_csv("4.output//data_prep_ver4.csv")
...
data_prep_ver5 %>% write_excel_csv("4.output//data_prep_ver5.csv")

# 3. 탐색적 분석 -------------------------------------------

read_csv("4.output//data_prep_ver5.csv") -> data_prep_ver5
...
data_prep_ver6 %>% 
  mutate(
    content = content %>% 
      ...
      str_replace_all("[^가-힣 ]", " ") # 한글만 추출
  ) -> data_prep_ver6

# 4.2. 사용자 사전 --------------------------------------
# 4.2.1. 빅카인즈 참고 --------------------------------------

readxl::read_excel("1.data//1.2.bigkinds.xlsx") -> bigkinds
bigkinds %>% 
  select(`인물`, `기관`) %>% 
  rename(person = `인물`,
...
data_word %>%  write_excel_csv("4.output//data_word.csv") # 중간저장


# 4.4. 사후 전처리 -----------------------------------------
read_csv("4.output//data_word.csv") -> data_word
...
data_word_prep2 %>% 
  write_excel_csv("4.output//data_final.csv") # 최종 단어 데이터
...
read_csv("4.output//data_final.csv") -> data_final
...
# 5.1.4. 단어 중심으로 데이터 통합: 이해충돌방지법 시기 ------------------
...
data_final_first %>%
  ...
  write_excel_csv("4.output//table.csv")
...
data_topics %>% write_excel_csv("4.output//data_topics.csv")
read_csv("4.output//data_topics.csv") -> data_topics
data_topics %>% 
  group_by(topic) %>% 
  slice_max(beta, n = 30) %>%
  ungroup() %>%
  arrange(topic, -beta) -> data_topic_terms
data_topic_terms %>% write_excel_csv("4.output//topic.csv")
